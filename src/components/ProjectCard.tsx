...
export default function ProjectCard({ project, onExpand }: ProjectCardProps) {
  return (
    <div className="bg-surface border border-border rounded-lg p-6 hover:border-accent hover-lift transition-colors fade-in">
      ...
      <p className="text-sm text-accent mb-4 italic">{project.hardProblem}</p>
      ...
      <button
        onClick={onExpand}
        className="w-full py-2 text-accent hover:text-accent/80 transition-colors text-sm font-medium"
      >
        Learn more →
      </button>
    </div>
